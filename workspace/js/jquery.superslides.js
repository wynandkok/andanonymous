...
(function() {
  ...
  Superslides = function(el, options) {
    ...
    var refreshDom = function() {
      $('#description-container').perfectScrollbar(function() {
        console('DOM refreshed');
      });
    }
    toggleNav = function() {
      if (_this.size() > 1) {
    ...
    parseHash = function(hash) {
      ...
      if (hash) {
        switch (hash) {
          ...
          case ('tricirccirc'):
            (function() {
              $("#tricirccirc").find('.ps-shad').fadeIn();
            })();
            return 1;
          case ('work'):
            return 2;
    ...
    animator = function(direction, callback) {
      var offset, outgoing_slide, position, upcoming_position, upcoming_slide, $mask, $stickyHeader;
      $mask = $("#mask");
      ...
      return $control.animate({
        ...
        var $outerW = $children.eq(upcoming_slide).outerWidth();
        var $innerW = $children.eq(upcoming_slide).find(".container").innerWidth();
        $stickyHeader.css({right: $outerW - $innerW});
        ...
        $stickyHeader.delay(2000).fadeIn("slow");
    ...
    this.stop = function() {
      ...
      console.log("slide animation stopped");
    ...
    $window.on('hashchange', function(e) {
      ...
      if (index == 0 || index == null) {
        $('#navContainer')
          .delay(700)
          .queue( function(next){
            $(this).removeClass('nav-dock').css({top: '50%'});
            next();
          });
      }
      else  {
