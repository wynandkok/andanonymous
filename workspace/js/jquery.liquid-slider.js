...
(function ($, window, document, undefined) {
  ...
  var Slider = {
    ...
    addNavigation: function () {
      ...
      // The id is assigned here to allow for responsive
        dropDownList,
        dynamicTabsElm = '<div id="navContainer"><' + self.options.navElementTag + ' class="liquid-nav"><div class="logo-mark"></div><div class="logo-nav" style="display: block;"><div class="logo-text"></div><div class="logo-tricirccirc"></div><ul id="' + (self.$elem).attr('id') + '-nav-ul"></ul></' + self.options.navElementTag + '>',
      ...
      if (self.options.responsive && self.options.mobileNavigation) {
        dropDownList = '<div class="liquid-slider-select-box"><select id="' + (self.$elem).attr('id') + '-nav-select" name="navigation">' + selectBoxDefault + '</select></div></div>';
      }
      ...
      $.each(
        ...
        function (n) {
          $((self.$sliderWrap)).find('.liquid-nav ul').append('<li class="tab' + (n + 1) + '"><a href="#' + (n + 1) + '" title="' + $(this).text() + '"></a></li>');
        }
      );
    ...
    updateHashTags: function (tab) {
      var self = this;
      if (!tab || tab == "0") {
        $('#navContainer').removeClass('nav-dock').css({top: '50%'});
        $("#slides").delay(9000).fadeIn();
      }
      else {
        $('#navContainer').addClass('nav-dock').css({top: '-10px'});
        $("#slides").fadeIn();
        $('#sticky_header').fadeIn('slow');
      }
  ...
  $.fn.liquidSlider.options = {
    ...
    preloaderElements: 'html,img,video,iframe,object,div',
